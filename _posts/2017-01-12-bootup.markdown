...
Currently, development environment requires the use of nightly
...
targeting an embedded environment. Xargo can be installed using
cargo.
...
`main` that can be called by the microcontroller. We now turn our
attention to laying out this code and data in the Teensy's flash
memory. This is done with a *linker script*.
...
At this point, we could compile our project and flash it to a
Teensy. Sadly, it would do nothing interesting - we wouldn't even be
able to tell if it was running. In the next section we'll expand our
code to do something more useful.
...
will try to minimize correctness errors - even if they're strictly
safe.
...
represent a single GPIO access. They will ensure safety by only
writing to the words associated with their pin index. Let's look at
...
code will not ever make a second copy of a pin that is in use as a
GPIO.
...
take advantage of our ability to panic to create APIs that use Rust's
type system to remove even the small amount of unsafe code currently
used.
