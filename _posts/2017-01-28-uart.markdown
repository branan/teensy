...
Even though updating these `Volatile` values is no longer unsafe, we
still need to keep this function marked as `unsafe`. Rust considers
certain interactions with packed structs to be `unsafe` since they can
...
[branch]: https://github.com/branan/teensy/tree/post_2
